...
from exceptions import ResponseDeniedError, ResponseKeyError, ResponseStatusError
...
def get_api_answer(current_timestamp):
    ...
    try:
        ...
        raise ConnectionError(ERROR_GET_API.format(error=err, **response_params))
    if response.status_code != HTTPStatus.OK:
        raise ResponseStatusError(
            ERROR_ENDPOINT.format(response=response.status_code, **response_params)
        )
...
def check_response(response):
    ...
    if not isinstance(response, dict):
        raise TypeError(ERROR_RESPONSE_TYPE.format(type=type(response)))
    if "homeworks" not in response:
        raise ResponseKeyError(ERROR_RESPONSE_KEY.format(key="homeworks"))
